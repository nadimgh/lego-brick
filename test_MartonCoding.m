%testing script for a Marton coding scheme (without precoding) over a two-user broadcast channel
...
rate_cell = getMaximumAchievableUserRates(H, P_dB, 'marton');
rate = getUserRates(sum_rate, rate_cell, 'marton');

target_dist = rate_cell{1}.input_dist_marton;
...
for i_p = 1:length(P)
    ...
    for i_user = 1:R
        idx0 = (x_all(i_user,:) == 1);
        mean0 = sqrt(P(i_p)/2) * H(i_user,:) * x_all(:,idx0);
        alpha0 = target_dist(i_p,idx0)/sum(target_dist(i_p,idx0));
        sigma0 = ones(1, 2^(R-1));
        
        idx1 = (x_all(i_user,:) == -1);
        mean1 = sqrt(P(i_p)/2) * H(i_user,:) * x_all(:,idx1);
    ...
    for realization = 1:num_realizations
        ...
        for i_user = 1:R
            y{i_user} = zeros(n, 1);
            for i_antenna = 1:T
                y{i_user} = y{i_user} + sqrt(P(i_p)/2) * H(i_user, i_antenna) * (1-2*double(x{i_antenna}));
            end
