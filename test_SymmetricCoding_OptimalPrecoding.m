%testing script for symmetric coding scheme with optimal precoding over a two-user broadcast channel
...
rate_cell = getMaximumAchievableUserRates(H, P_dB, 'symmetric_opt');
rate_1 = getUserRates(sum_rate, rate_cell, 'symmetric_opt');
rate_2 = flipud(rate_1);
...
tic
for i_p = 1:length(P)
    if (rate_cell{2}{i_p}(1) > rate_cell{2}{i_p}(2) && rate_1(1) > rate_1(2)) || (rate_cell{2}{i_p}(1) < rate_cell{2}{i_p}(2) && rate_1(1) < rate_1(2))
        rate = rate_1;
    else
        rate = rate_2;
    end
