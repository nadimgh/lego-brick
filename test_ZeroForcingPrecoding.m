%testing script for zero-forcing precoding scheme over a two-user broadcast channel
...
rate_cell = getMaximumAchievableUserRates(H, P_dB, 'zf');
rate = getUserRates(sum_rate, rate_cell, 'zf');

W_precoding = cell(1, length(P));
P_opt = cell(1, length(P));
for i_p = 1:length(P)
    P_opt{i_p} = getOptimalZeroForcingPowerAllocation(H, P(i_p));
    W_precoding{i_p} = H\sqrt(diag(P_opt{i_p})); % Zero forcing precoding matrix
end
